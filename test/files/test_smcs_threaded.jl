...
@testset "Sequential MCS, 100 samples, RBTS, threaded" begin
    ...
    #120.980365 seconds (269.38 M allocations: 19.098 GiB, 5.07% gc time, 27.59% compilation time: 1% of which was recompilation)
    system_EENS_mean_no_storage = [0.0, 0.0, 15.32895, 0.0, 0.19304, 123.33102]
    ...
    method = CompositeAdequacy.SequentialMCS(samples=100, seed=100, threaded=true)
    @time shortfall_threaded,_ = CompositeSystems.assess(system, method, settings, resultspecs...)
    #802.625093 seconds (1.27 G allocations: 79.037 GiB, 3.37% gc time, 0.08% compilation time)
    system_EENS_mean_storage = [0.0, 0.0, 9.18234, 0.0, 0.76072, 116.98534]
