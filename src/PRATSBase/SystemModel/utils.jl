...
function container(container_key::Vector{<:Any}, key_order::Vector{<:Any}, dictionary_core::Dict{<:Any}, 
    ...
    @assert length(dictionary_timeseries) == length(network.load)
    container_bus = [Int64.(network.load[i]["load_bus"]) for i in keys(dictionary_timeseries)]
...
function container(container_key::Vector{<:Any}, key_order::Vector{<:Any}, dictionary_core::Dict{<:Any}, 
    ...
    if length(container_key) ≠ length(container_key_core)
        for i in container_key_core
            if in(container_key).(i) == false
                setindex!(dictionary_timeseries, Float16.([network.gen[i]["pg"]*network.baseMVA for k in 1:N]), i)
            end
        end
    ...
    container_data = [Float16.(dictionary_timeseries[i]/network.baseMVA) for i in keys(dictionary_timeseries)]
    container_bus = Int.(reduce(vcat, tmp')[:,2])
...
function container(container_key::Vector{<:Any}, key_order::Vector{<:Any}, dictionary_core::Dict{<:Any}, 
    ...
    tmp = sort([[i, Int.(branch["f_bus"]), Int64.(branch["t_bus"]), Float16.(branch["rate_a"]),
                Float16.(branch["rate_b"])] for (i,branch) in network.branch], by = x->x[1])
    ...
    if length(container_key) ≠ length(container_key_core)
        for i in container_key_core
            if in(container_key).(i) == false
                setindex!(container_longterm_capacity, Float16.([network.branch[i]["rate_a"] for k in 1:N]), i)
                setindex!(container_shortterm_capacity, Float16.([network.branch[i]["rate_b"] for k in 1:N]), i)
            end
        end
    ...
    container_f_bus = Int.(reduce(vcat, tmp')[:,2])
    container_t_bus = Int.(reduce(vcat, tmp')[:,3])
    container_λ = Float32.(values(dictionary_core[Symbol("failurerate[f/year]")]))
    container_μ = Vector{Float32}(undef, length(values(dictionary_core[Symbol("repairtime[hrs]")])))
